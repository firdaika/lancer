...
<div class="sidebar">
        ...
        <a href="{{ url('/dataMateri') }}">Data Materi</a>
         <a href="{{ url('/jadwalAdmin') }}">Data Jadwal</a>
          ...
          <a href="{{ url('/dataGuru') }}"  class="active">Data Guru</a>
          <a href="{{ url('/dataPaket') }}" >Data Paket</a>
         <a href="{{ url('/notifikasi') }}">Notifikasi</a>


       <!-- Tombol Logout tepat di bawah pengaturan -->
...
<div class="main-content">
  ...
  <div class="header">
    ...
    <button class="btn-add" id="openAdd">+ Tambah Guru</button>
  </div>
      ...
      <tr data-id="{{ $g->id }}">
        ...
        <td>
          <button class="action-btn edit openEdit"
                  ...
                  data-nama="{{ $g->nama }}"
                  data-nip="{{ $g->nip }}"
                  data-bidang="{{ $g->bidangKeahlian }}"
                  data-telfon="{{ $g->telpon }}"
                  data-ttl="{{ $g->ttl }}">
            ✏️
          </button>
...
<div id="guruModal" class="modal">
  <div class="modal-content">
    <form id="guruForm" method="POST">
      ...
      <input type="text" id="nama" name="nama"  placeholder="Nama Guru" required>
      <input type="text" id="nip" name="nip"  placeholder="NIP" required>
      <input type="text" id="bidangKeahlian" name="bidangKeahlian" placeholder="Bidang Keahlian">
      <input type="text" id="telfon" name="telpon" placeholder="Nomor Telfon">
      <input type="text" id="ttl" name="ttl" v placeholder="Tempat, Tanggal Lahir">
...
openAddBtn.addEventListener("click", () => {
  ...
  document.getElementById("bidangKeahlian").value = "";
...
document.querySelectorAll(".openEdit").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    // <-- FIX: tambahkan slash sebelum id
    document.getElementById("guruForm").action = "/dataGuru/update/" + id;
    document.getElementById("methodOverride").value = "";

    document.getElementById("nama").value = btn.dataset.nama || '';
    document.getElementById("nip").value = btn.dataset.nip || '';
    document.getElementById("bidangKeahlian").value = btn.dataset.bidang || '';
    // <-- pastikan dataset dan input id sama: gunakan 'telfon'
    document.getElementById("telpon").value = btn.dataset.telpon || '';
    document.getElementById("ttl").value = btn.dataset.ttl || '';
  });
});
